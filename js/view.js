(function($) {
	...
	window.renderText = function(x, y, fontSize, color, text, font) {
		ctx.save();
		if (!font) {
			var font = 'px Exo';
		}
	...
	window.drawScoreboard = function() {
		if (scoreOpacity < 1) {
			scoreOpacity += 0.01;
			textOpacity += 0.01;
		}
	...
	window.toggleClass = function(element, active) {
	...
	window.setMainMenu = function() {
		...
		$('#restartBtn').hide();
		var pauseSrc = $("#pauseBtn").attr("src") || "";
		if (pauseSrc.indexOf("btn_pause.svg") !== -1) {
			$("#pauseBtn").attr("src", getImageUrl('btn_resume.svg'));
		} else {
	...
	window.updateHighScores = function(){
		$("#cScore").text(score);
	...
	window.pausable = true;
	window.pause = function(o) {
