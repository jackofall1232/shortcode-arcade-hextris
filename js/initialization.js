(function($) {
	...
	window.initialize = function(a) {
		...
		window.op = 0;
		window.saveState = safeGetItem("saveState") || "{}";
		if (saveState !== "{}") {
			op = 1;
		}
		...
		highscores = [];
		var storedScores = safeGetItem('highscores');
		if (storedScores) {
			try {
		...
		if (a != 1) {
			...
			document.addEventListener("menubutton", handlePause, false);
	...
	window.handleTapBefore = function(e) {
	...
	window.handleClickBefore = function(e) {
		var x = e.clientX;
		var y = e.clientY;
